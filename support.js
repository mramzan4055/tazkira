...
(() => {
  ...
  function createRuntime(doc = document) {
    ...
    function ensureFetched(name) {
      ...
      if (r.fetched) return;
      r.fetched = true;
      const htmlUrl = COMPONENT_DIR + "/" + encodeURIComponent(name) + ".html";
      const dcUrl = COMPONENT_DIR + "/" + encodeURIComponent(name) + ".dc.html";
      const res = window.__resources;
      const pre = res ? (res[htmlUrl] || res[dcUrl]) : void 0;
      const target = typeof pre === "string" && pre ? pre : htmlUrl;
      const blob = bundledBlob(target);
      const fetchPromise = blob ? blob.text() : fetch(target).then((res2) => {
        if (res2.ok) return res2.text();
        return fetch(dcUrl).then((res3) => res3.ok ? res3.text() : "");
      });
      fetchPromise.then((t) => {
        if (!t) return;
